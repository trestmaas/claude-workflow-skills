...
- **A subagent killed by an API rate limit before its first push lost everything.** Two SIGN-1303 lifecycles died that way on participant-questions; the third started from zero. The harness reports it as an agent error naming `rate_limit` / HTTP 429. Do not respawn into the same limit — that burns another lifecycle for the same result; pause, report the limit to the user, and resume when it clears (a model switch cleared it there). The `/start`-side mitigation is a WIP push right after the red-tests commit, so a killed agent's branch is on the remote for the respawn to continue from. **After the first rate-limit kill in a run, drop concurrency to 1 for the rest of the run**, and tell every brief to commit and push the moment anything goes green. Rate limits are a burn-rate signal, and parallel authors plus reviewers are what hits them. When a killed agent had uncommitted work, resume it (SendMessage) and tell it to `git diff --stat` and continue rather than reset. On self-hosted-ci-runners three agents were killed mid-fix and all three resumed with nothing lost that way.
- **Run the orchestrator itself on Sonnet** (`/model` before `/project-start`). It reads reports, patches tickets, writes briefs and runs `gh`. It is the longest-lived context in the run, and every turn re-reads all of it, so an Opus/1M orchestrator can out-spend any single child. Its judgement calls (adjudicating a review, picking a fix option) are bounded by the reviewer's evidence.
- **NITs from an independent review default to follow-up tickets, not the fix round.** Send the owning agent only BLOCKING findings, plus NITs that are one-line and on lines it is already changing. Each fix round resumes the author, reloading a 200–350k transcript, so folding a dozen NITs in multiplies that cost. File the rest as one follow-up ticket per PR, listing the NITs with file:line, in the evergreen follow-up flow or the project's backlog.
...
2. Spawn a **fresh** `general-purpose` agent (NOT `run_in_background`-coupled to the author; a clean context that did not write the code) on the strongest model, whatever the author ran on, as a read-only reviewer: "you did not write this, find what its author missed." This gate is what makes cheaper authors safe, so don't downgrade it. Point it at the PR's actual risk surface and tell it to distinguish BLOCKING from "I'd have done it differently." Have it end with a single `VERDICT:` line.
   ...
   **Brief the reviewer on the NEIGHBOURS, not just the diff.** This is the part that does the work, and it is the orchestrator's alone — only it knows what the other twelve tickets did. A reviewer confined to the diff re-checks what the author already checked; the defects that survive self-review live in the *seams*. Put in the brief:
   ...
   - **Every "no behaviour change" / "no widening" claim, checked against the NARROWEST path that reaches the operation today** — *re-derive, don't accept*. On One Actor the orchestrator's own scope note said "any membership is a superset of owner — no widening"; true of the router it had read, false of the id-keyed procedures, which on `main` were reachable *only* through the caller's personal org. The result was a plain org member able to rotate a team org's webhook signing secret. Before writing such a claim — and when reviewing one — enumerate current reachability *per Transport, per operation* ("who can reach this today, through what"), not per helper.
   - **Every "N doors/writers/callers" count the ticket or PR body states, with the grep that enumerates it** — *re-run, don't accept*. On Access Token two such counts were wrong (7 callers → 8; 2 writers → 3) and the reviewer's re-run is what found the third writer.
   - **Every claim about what a RED run proved, checked against what the RED run could structurally produce** — *re-derive, don't accept*. On Dashboard Actor a PR body listed three failure causes for its red-before-green run; one occurred **zero** times and could not occur, because on the parent the Actor lands in the `input` position, so the old helper threw its own `NOT_FOUND` before ever reaching the `FORBIDDEN` being claimed. The run proved the call-shape change and two closed oracles; it proved nothing about the refusal displacement, which was demonstrated elsewhere. Ask of every RED proof: *which of these failures is the mechanism under test, and which is the harness failing earlier?* A red suite is evidence of **a** break, not of **the** break.
   - **Every mutant the PR adds, checked that the mutated tree still compiles and runs** — a mutant whose `replace` introduces an identifier that does not exist crashes the file, fails every test in it, satisfies its RED expectation, and proves nothing. Three such mutants were found on one project. **The tell: a crashing mutant fails its *positive* cases too, which a behavioural mutant never does.** Ask for the failure count and the named case, not just "RED".
   - **Every guard, ratchet or scanner the PR ships, attacked rather than read** — write your own evasion probes and report which the guard catches. The house set, all of which a real guard survived: the call split across a line break, `const { name } = obj` destructuring, template-literal and computed access, a `.tsx` file, a directory the project never touched, a re-export, a **near-name file trying to inherit the self-exemption**, and narrowing the guard's own ignore list. An evasion a plausible future author would actually write is a finding; deliberate obfuscation is a NIT.
...
```
[project-start] N/T done / M in-flight / K ready / B blocked / P paused — released <TICKET-ID> → <TICKET-ID> started
```

`T` is the project's total ticket count; `B` is tickets still waiting on unmerged dependencies. The five buckets must sum to `T` — a line that doesn't add up is hiding a ticket. Keep this terse. The user is hands-off but should be able to glance.
...
- Does not edit `tickets.yaml` mid-run. Plan is fixed once execution begins; if you discover the plan was wrong, pause and re-plan. **One exception, and it is narrow: the orchestrator may add a ticket to the project mid-run when all three hold — (a) an independent review names the work, (b) its dependencies are already merged or are the ticket it splits from, and (c) the closing ticket's claim is false without it.** Record the addition as a decision (what the review found, why it is not a follow-up outside the project) and brief it like any other ticket. On One Actor that was SIGN-1625: the reviewer proved the dashboard's participant-remove still diverged from the unified one, which would have made the closing ticket's "every organizer operation is one method" false; it shipped 1h17m later with no findings. Anything failing (a)–(c) is a follow-up ticket outside the project, not a DAG edit.
